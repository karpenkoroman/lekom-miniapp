(() => {
  ...
  $('#goPoll')?.addEventListener('click', ()=> showOnly(scrPoll));
  backFromPoll?.addEventListener('click', ()=> showOnly(scrStart));
  openResultFromStart?.addEventListener('click', ()=> showOnly(scrResult));
  backFromResult?.addEventListener('click', ()=> { updateStartResumeCta(); showOnly(scrStart); });
