...
// Оставить контакты — раскрыть и прокрутить
...
document.getElementById('sendLead').onclick = async ()=>{
  ...
  const box=document.getElementById('resultText');
  box.innerHTML = `<b>Спасибо!</b> Контакты отправлены.<br><div style="margin-top:8px;">
    <a href="https://t.me/LekomIT" target="_blank" class="btn btn-primary" style="display:inline-block;width:auto;">📢 Подписаться на @LekomIT</a>
  </div>`;
};

// Обсудить с экспертом — центр. подсказка
document.getElementById('ctaExpert').onclick = async ()=>{
  ...
  // прямой переход можно сделать отдельной кнопкой внутри модалки, но по ТЗ — только подсказка
};
